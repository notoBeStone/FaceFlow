...
class PriceView: UIView {
    private(set) var selectedType: Price27616Type = .default {
        didSet {
    ...
    var handler: ((_ type: Price27616Type)->())?
    ...
    private lazy var itemViews: [PriceItemView] = {
        let views: [PriceItemView] = Price27616Type.allCases.map { type in
...
class PriceItemView: UIView {
    ...
    func showPrice() {
        ...
        switch self.type {
            case .year:
                ...
                topText = self.type.price?.price ?? "--"
            case .week:
                text = "conversionpage_price_xdays".localized(for: Vip27616AContentView.self, tableName: "Localizable27616", "\(self.type.trailDays)")
                topText = "conversionpage_price_free".vipLocalized()
        }
    ...
    private lazy var topLabel: UILabel = {
        ...
        switch self.type {
            ...
            case .week:
                text = "conversionpage_price_free".vipLocalized()
        }
        label.text = text
        return label
    }()
